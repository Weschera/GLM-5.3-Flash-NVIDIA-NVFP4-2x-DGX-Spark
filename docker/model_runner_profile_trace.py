...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def profile_run(self) -> None:
        ...
        # Diagnostic only: trace each decoder layer during the startup profile.
        # Hooks are removed before any real inference or throughput measurement.
        import json as _json
        import re as _re
        from pathlib import Path as _Path

        def _trace(_name, _stage):
            _available = next(
                int(_line.split()[1]) * 1024
                for _line in _Path('/proc/meminfo').read_text().splitlines()
                if _line.startswith('MemAvailable:')
            )
            print('PROFILE_MEMORY ' + _json.dumps({
                'layer': _name, 'stage': _stage,
                'num_tokens': self.max_num_tokens,
                'allocated': torch.cuda.memory_allocated(),
                'reserved': torch.cuda.memory_reserved(),
                'host_available': _available,
            }), flush=True)

        _hooks = []
        for _name, _module in self.model.named_modules():
            if _re.search(r'(?:^|\.)layers\.\d+$', _name):
                _hooks.append(_module.register_forward_pre_hook(
                    lambda _m, _a, _n=_name: _trace(_n, 'before')))
                _hooks.append(_module.register_forward_hook(
                    lambda _m, _a, _o, _n=_name: _trace(_n, 'after')))
        _trace('profile', 'before')
        try:
